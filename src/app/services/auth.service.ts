...
export class AuthService {
  ...
  refreshToken(): Observable<LoginResponse | null> {
    ...
    return this.http.post<LoginResponse>(`${this.apiUrl}/auth/refresh-token`, {refresh_token}, this.CONTEXT)
      .pipe(
        catchError(() => of()),
        tap(data => {
          const refresh_token = localStorage.getItem('refresh_token');
  ...
  login(body: ILogin): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${this.apiUrl}/auth/signin`, body, this.CONTEXT)
      .pipe(
        catchError(error => {
          if(error.status == 401) {
  ...
  logout(): void {
    ...
    this.http.post<LoginResponse>(`${this.apiUrl}/auth/refresh-token`, {refresh_token}, this.CONTEXT)
